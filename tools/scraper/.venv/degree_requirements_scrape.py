...
URL = "https://catalog.unh.edu/graduate/academic-regulations-degree-requirements/degree-requirements/"
...
def push_section(level, title):
    ...
    while stack and stack[-1][0] >= level:
        stack.pop()
    parent = stack[-1][1]
    new_node = {"type": "section", "title": title, "content": []}
...
def add_paragraph(tag):
    ...
    links = [{"label": clean_text(a), "url": urljoin(URL, a["href"].strip())}
...
for el in flow:
    ...
    if lvl:  # if it's a heading
        title = clean_text(el)
        if lvl == 2:
            toc.append(title)  # top-level only
        push_section(lvl, title)
    else:
...
def normalize(section_node):
    ...
    out = {}
    out["title"] = title
    if text_blocks: out["text"] = text_blocks
    if list_blocks: out["lists"] = list_blocks
    if link_bucket: out["links"] = link_bucket
    if children: out["subsections"] = children
    return out

print("[info] assembling JSON...")
page_title = clean_text(soup.find("h1")) if soup.find("h1") else "Degree Requirements"
